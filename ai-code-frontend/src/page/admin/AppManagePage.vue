    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.dataIndex === 'userVo'">
          <UserInfo :userVo="record.userVo" size="small" />
        </template>
...
import { deleteAdminApp, getAdminAppVoListPage, updateAdminApp } from '@/api/appController'
...
const fetchData = async () => {
  try {
    const res = await getAdminAppVoListPage({
      ...searchParams,
    })
    if (res.data.data) {
...
const deleteApp = async (id: number | undefined) => {
  ...
  try {
    const res = await deleteAdminApp({ id })
    if (res.data.code === 0) {
